        ...
        <div className="lg:grid lg:grid-cols-[240px_minmax(0,680px)] lg:gap-12 max-w-5xl">
          ...
          <main className="prose-doc">
            ...
            <Section id="winrate" title="Win rate calculation">
              <p>
                Faction- and build-level win rates aren't raw <InlineMath math="W/(W+L)" /> — they're <strong>time-weighted</strong> (recent meta dominates while old games contribute weakly), <strong>draw-inclusive</strong> (a draw counts as half a win, unlike the raw rate which ignores draws), and <strong>empirical-Bayes shrunk</strong> toward 50% so a 5-0 build at <em>n</em>=5 doesn't read as confidently as a 5-0 build at <em>n</em>=200. They are also <strong>adjusted for opponent strength</strong>: each game is scored against what the player's rating said to expect, so beating strong opponents counts for more than beating weak ones. Player ratings carry across editions — a pilot's ability is theirs, not their codex's — which means 92% of games have both players rated rather than sitting at a default. The correction moves a typical build by about <strong>2 points</strong> and the most pilot-dependent ones by <strong>10 or more</strong>: a build showing 77% raw sits at 65% once you account for who was flying it. That gap is the whole reason the adjustment exists.
              </p>
              ...
              <BlockMath math="w_g = \exp\!\bigl(-d_g / 45 \bigr)" />
              <p>
                where <InlineMath math="d_g" /> is the game's age in days. The 45 days is the decay <em>constant</em>, not the half-life: weight is <InlineMath math="e^{-d/45}" />, so the half-life is about <strong>31 days</strong>. A game from yesterday counts 1.0×; one from 31 days ago counts 0.5×; from 60 days ago 0.26×; from six months ago ~0.02×. Pre-dataslate games still inform the estimate but don't drown out current-meta signal.
              </p>
              <p>
                There is deliberately <em>no</em> post-dataslate blackout. An earlier version zeroed a fortnight of games after each balance update, on the reasoning that the first weekends are noisy. We measured it instead: against three fabricated patch boundaries, faction and build win rates moved no more at the real one than at the fakes. What <em>did</em> move was composition — what people bring changes almost immediately, while how often a list wins does not. So a patch is not a reason to discard results, and age decay already gives recent games more say.
              </p>
            ...
            <Section id="pushback" title="Where to push back">
              ...
              <ol className="list-decimal pl-5 space-y-2 mt-3">
                ...
                <li>
                  <strong>Recency decay.</strong> 45-day decay constant (≈31-day half-life) for the headline win rate, and no post-dataslate blackout — games decay with age, they are never zeroed for falling near a patch. 60-day half-life on example-list ranking. 30 vs 60 day windows for the momentum signal. These are calibrated defaults from a reviewer's-thumbs-sideways consensus, not theorems — if you watched the meta turn over the Q1 2026 dataslate and you think we're reading the transition wrong, tell us what you saw and where we look stale.
                </li>
                <li>
