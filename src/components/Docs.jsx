        ...
        <div className="lg:grid lg:grid-cols-[240px_minmax(0,680px)] lg:gap-12 max-w-5xl">
          ...
          <main className="prose-doc">
            ...
            <Section id="winrate" title="Win rate calculation">
              <p>
                Faction- and build-level win rates aren't raw <InlineMath math="W/(W+L)" /> — they're <strong>time-weighted</strong> (recent meta dominates while old games contribute weakly), <strong>draw-inclusive</strong> (a draw counts as half a win, unlike the raw rate which ignores draws), and <strong>empirical-Bayes shrunk</strong> toward 50% so a 5-0 build at <em>n</em>=5 doesn't read as confidently as a 5-0 build at <em>n</em>=200. There is also an opponent-strength correction, but be sceptical of it: 11th edition is young, roughly 80% of games are a player's first event in our data, and the correction currently moves a build's rate by <strong>under 1 percentage point</strong>. It is not currently doing the job of separating a strong pilot from a strong list, and we would rather say so than imply a control we don't have.
              </p>
